...
TypeId DashFileDownloader::GetTypeId(void) {
...
DashFileDownloader::DashFileDownloader(): m_running(false), m_serverPort(0), m_numDownloaded(0), m_count(1), m_clen(102400), m_traceId(0) {
...
void DashFileDownloader::StopApplication() {
	NS_LOG_FUNCTION(this);
	if(!m_running) return;
...
void DashFileDownloader::EndApplication() {
	NS_LOG_FUNCTION(this);
	if(!m_running) return;
	m_running = false;
	LogTrace();
...
void DashFileDownloader::StartApplication() {
	NS_LOG_FUNCTION(this);
	NS_ASSERT(!m_running);
...
void DashFileDownloader::StartDash() {
	NS_LOG_FUNCTION(this);
	if(!m_running) return;

	Ptr<Ipv4L3Protocol> ipnode = GetNode()->GetObject<Ipv4L3Protocol>();
	ipnode->TraceConnectWithoutContext("Tx", MakeCallback(&DashFileDownloader::TxTracedCallback, this));
	ipnode->TraceConnectWithoutContext("Rx", MakeCallback(&DashFileDownloader::RxTracedCallback, this));

	m_httpDownloader = Create<HttpClientBasic>();
	m_httpDownloader->SetCollectionCB(MakeCallback(&DashFileDownloader::DownloadedCB, this).Bind(Ptr<Object>()), GetNode());

	if(m_nodeTraceCB.IsNull())
		m_nodeTraceCB = MakeCallback(&DashFileDownloader::CollectNodeTrace, this);
	DownloadNextSegment();
}

void DashFileDownloader::DownloadNextSegment() {
	NS_LOG_FUNCTION(this);
	if(!m_running) return;

	if(m_node) {
		m_nodeTraceFp.open(m_nodeTracePath + "_" + std::to_string(GetNode()->GetId())
								+ "_" + std::to_string(m_numDownloaded) + ".csv", std::ofstream::out | std::ofstream::trunc);
		m_nodeTraceFp << "#,Time,rxSpeed,txSpeed," << m_nodeTraceCB(GetNode(), true) << std::endl;
		m_traceId = 0;
	}
	const clen_t expected = m_clen; //Arbit
	m_httpDownloader->InitConnection(m_serverAddress, m_serverPort, "/dl-" + std::to_string(m_clen));
	m_httpDownloader->AddReqHeader("X-Require-Length", std::to_string(expected));
	m_httpDownloader->Connect();
}

void DashFileDownloader::Downloaded() {
	NS_LOG_FUNCTION(this);
	if(!m_running) return;
...
void DashFileDownloader::DownloadedCB(Ptr<Object> obj) {
	NS_LOG_FUNCTION(this);
	Simulator::ScheduleNow(&DashFileDownloader::Downloaded, this);
}

void DashFileDownloader::PeriodicDataCollection() {
	NS_LOG_FUNCTION(this);
	if(!m_nodeTraceFp.is_open())
		return;
	...
	m_nodeTraceFp << m_traceId << "," << now.GetSeconds() << "," << rxSpeed << "," << txSpeed << "," << m_nodeTraceCB(node, false) << std::endl;
	m_traceId ++;
	Simulator::Schedule(m_nodeTraceInterval, &DashFileDownloader::PeriodicDataCollection, this);
}

std::string DashFileDownloader::CollectNodeTrace(Ptr<Node> node, bool firstLine) {
	if(firstLine)
		return "nodeId";
	return std::to_string(node->GetId());
}

void DashFileDownloader::TxTracedCallback(Ptr<const Packet> packet,
		Ptr<Ipv4> ipv4, uint32_t interface) {
	m_txData[interface] += packet->GetSize();
}

void DashFileDownloader::RxTracedCallback(Ptr<const Packet> packet,
		Ptr<Ipv4> ipv4, uint32_t interface) {
	m_rxData[interface] += packet->GetSize();
}

void DashFileDownloader::LogTrace() {
	NS_LOG_FUNCTION(this);
